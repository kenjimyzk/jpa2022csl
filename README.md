...
`sort_json.py` は `pykakasi` ライブラリを使用して漢字・カタカナを自動的にひらがなに変換するため、**基本的に `yomi` の入力は不要です**。

ただし、人名の読みが一般的でない場合や変換結果が正しくない場合には、Zoteroの「その他」欄に以下の形式で読み仮名を明示的に指定できます。
...
```bash
# uvがある場合（推奨）：依存パッケージを自動インストールして実行
uv run sort_json.py

# または pykakasi をインストール済みの環境で
python3 sort_json.py
```

このスクリプトは以下の処理を行います：
*   日本語文献を自動検出し、著者名を `pykakasi` でひらがなに変換して `curator` フィールドに格納します。
*   `yomi` が明示されている場合はそちらを優先します。
*   ひらがなのUnicodeコードポイントはあいうえお順に対応しているため、正しく五十音順にソートされます。
