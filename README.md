...
Zoteroの「その他」欄に、以下の形式で著者の読み仮名（ローマ字）を入力してください。
これがJSONエクスポート時に `note` フィールド内の `yomi` 変数として出力され、スクリプトによって `curator` フィールド（ソート用キー）に変換されます。

```
yomi: FamilyName, GivenName
```

例：
```
yomi: Hasegawa, Toshikazu
yomi: Miyazaki, Kenji
```
※ 複数著者がいる場合は、第一著者の読みだけで十分な場合が多いですが、正確を期すなら全員分記述することを推奨します（ただし現状のスクリプトは第一著者の読みのみをソートに使用する場合があります）。

その後、Zoteroから `Better CSL JSON` 形式でエクスポートします（例: `opensource.json`）。
...
このスクリプトは以下の処理を行います：
*   日本語文献を自動検出し、`yomi` フィールドの情報を使ってローマ字の読みを `curator` フィールドに生成します。
*   `yomi` がない文献については、スクリプト内の `kanji_map` 辞書を参照して変換を試みます（辞書にない場合は漢字のままとなり、正しくソートされません）。
